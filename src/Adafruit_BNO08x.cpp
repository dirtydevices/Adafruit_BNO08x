 ...
 */
bool Adafruit_BNO08x::begin_I2C(uint8_t i2c_address, TwoWire *wire, int32_t sensor_id) {
    if (i2c_dev) {
...
/**
 * @brief Enable the given report type with wake-up and always-on control
 *
 * @param sensorId The report ID to enable
 * @param interval_us The update interval for reports to be generated, in
 * microseconds
 * @param wakeupEnabled If true, the sensor wakes the host on event
 * @param alwaysOnEnabled If true, the sensor remains on in sleep state
 * @return true: success false: failure
 */
bool Adafruit_BNO08x::enableReport(sh2_SensorId_t sensorId, uint32_t interval_us, bool wakeupEnabled, bool alwaysOnEnabled) {
    static sh2_SensorConfig_t config;

    config.changeSensitivityEnabled = false;
    config.wakeupEnabled = wakeupEnabled;
    config.changeSensitivityRelative = false;
